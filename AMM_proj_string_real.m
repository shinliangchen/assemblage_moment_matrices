function string_moment_matrix = AMM_proj_string_real(Seq)
%%%%%%
%
...
for i = 1:length(Seqdag)
    for j = 1:length(Seq)
        ...
        Si_and_Sj_B_B_short_flip = flip(Si_and_Sj_B_B_short);

        if is_zero == 1
            string_moment_matrix(i,j) = string('0');
        elseif ~or(any(any(strjoin(Si_and_Sj_B_B_short,'*') == string_moment_matrix)) ,...
                any(any(strjoin(Si_and_Sj_B_B_short_flip,'*') == string_moment_matrix)))
            string_moment_matrix(i,j) = strjoin(Si_and_Sj_B_B_short,'*');
        elseif any(any(strjoin(Si_and_Sj_B_B_short,'*') == string_moment_matrix))
            string_moment_matrix(i,j) = strjoin(Si_and_Sj_B_B_short,'*');
        elseif any(any(strjoin(Si_and_Sj_B_B_short_flip,'*') == string_moment_matrix))
            string_moment_matrix(i,j) = strjoin(Si_and_Sj_B_B_short_flip,'*');
        else
            error('something is wrong')
        end
